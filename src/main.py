        ...
        try:
            ...
            for row in range(2, ws.max_row + 1):
                if ws.cell(row, 1).value and ws.cell(row, 2).value:
                    excel_dict.update(
                        {str(ws.cell(row, 1).value): str(ws.cell(row, 2).value)}
                    )
            wb.close()
            return excel_dict
...
class EnmRestApi(Session):
    def __init__(self, url, login, password):
        ...
        self.url = url
        self.verify = False
        self.form = {"IDToken1": login, "IDToken2": password}
    ...
    def check_login(self):
        try:
            ...
            if response.status_code != requests.codes.ok:
                logging.error(
                    f"incorrect login or password. status_code {response.status_code}"
                )
    ...
    def get_poid(self, element):
        ...
        mo_type = ""
        url = (
            f"https://enm.telecom.com/managedObjects/temporaryQueryForMoClassMapping/v2?query="
    ...
    def get_pos_by_poids(self, element_poid, port_poid, mo_type, element):
        ...
        try:
            ...
            if type(resp_dict) is dict:
                logging.critical(
                    f"exception in get_pos_by_poids element: {element}\n resp_dict: {resp_dict}"
                )
            ...
            data = {
                ...
                "defaultMappings": ["syncStatus"],
                "attributeMappings": [
                    {"moType": mo_type, "attributeNames": ["port", "ipAddress"]}
                ],
            }
            response = self.post(url, data=json.dumps(data), timeout=60)
            resp_dict = response.json()
    ...
    def cli_app(self, command):
        ...
        def encode_multipart_formdata(fields):
            ...
            content_type = (
                "multipart/form-data; boundary=----WebKitFormBoundary%s" % boundary
            )
...
def main():
    error_bs = "Error BS: "
    print(
        "Put the file with name bs.xlsx in the current directory with this script and after, "
    ...
    if enm.check_login():
        if os.path.isfile(
            os.path.normpath(os.path.dirname(__file__)) + "\\" + "bs_all.txt"
        ):
        ...
        for old_bs, new_bs in excel_dict.items():
            ...
            attributes, port = enm.get_pos_by_poids(
                element_poid, port_poid, mo_type, element
            )
            if attributes:
